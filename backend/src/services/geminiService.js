...
You have full knowledge of every page and feature.

PLATFORM PAGES & FEATURES:
1. /dashboard - Live KPIs, attack charts, recent activity
2. /attacks - Full attack event table with Forensics and Report buttons
3. /alerts - System alerts with priority badges
4. /logs - Raw request logs
5. /pcap - PCAP network capture analyzer
6. /action-queue - AI-suggested remediation actions pending human approval
7. /audit - Immutable audit log
8. /services - Service health dashboard
9. /copilot - AI Copilot Q&A (you are here)
  ...
  for (let m = 0; m < models.length; m++) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        if (is404) { logger.warn(`[GeminiService] 404: ${modelName} not found`); break; }
        if (is429 && attempt === 1) {
        ...
        if (is429 && attempt === 2) { logger.warn(`[GeminiService] ${modelName} still rate-limited`); break; }
        throw err;
      }
    }
  }
...
const STEPS_FORMAT_INSTRUCTION =
  `If your answer involves taking an action in the SENTINAL UI, include a STEPS: block.`;
...
async function chat(question, recentAttacks, history = []) {
  ...
  try {
    ...
    if (sugMatch) { try { suggestions = JSON.parse(sugMatch[1]); } catch {} answer = answer.replace(/SUGGESTIONS:\s*\[[^\]]*\]/s, '').trim(); }
...
async function* chatStream(question, recentAttacks, history = []) {
  if (!getModels()) { yield { type: 'error', errorCode: 'NO_API_KEY' }; return; }
  const { context, indexedIds } = buildAttackContext(recentAttacks);
  const historyBlock = buildHistoryBlock(history);
  const prompt =
    `You are SENTINEL AI, a senior cybersecurity analyst embedded in the SENTINAL threat detection platform.\n` +
  ...
  try {
    ...
    for await (const chunk of generateStreamWithFallback(prompt)) {
      fullText += chunk;
      if (!metadataStarted) { metadataStarted = /(?:^|\n)(?:SUGGESTIONS:|SOURCES:)/.test(fullText); }
      if (!metadataStarted) { yield { type: 'chunk', text: chunk }; }
    }
    let suggestions = [];
    let sourcedEventIds = [];
    ...
    if (err.isQuotaError) { yield { type: 'error', errorCode: 'QUOTA_EXHAUSTED' }; }
    else { logger.error(`[GeminiService] chatStream() failed: ${err.message}`); yield { type: 'error', errorCode: 'UNKNOWN_ERROR' }; }
  }
}

async function generateReport(attack, reportType = 'technical') {
  const staticReport = {
    generated: false, reportType,
    ...
    remediation_steps: [`Block IP: ${attack.ip || 'unknown'}`, 'Review last 24h of requests from this IP', `Update WAF rules for ${attack.attackType || 'this'} patterns`, 'Apply latest security patches'],
  ...
  const audienceInstructions = {
    executive: 'Write for a non-technical executive audience. Focus on business impact, risk, and strategic recommendations.',
    technical: 'Write for a security engineer. Include technical detail about the attack vector and precise remediation steps.',
  ...
  const prompt =
    ...
    `ATTACK: id=${attack._id} type=${attack.attackType} severity=${attack.severity} status=${attack.status} ` +
    `ip=${attack.ip || 'unknown'} confidence=${attack.confidence != null ? Math.round(attack.confidence * 100) + '%' : '?'} ` +
    `ts=${attack.timestamp ? new Date(attack.timestamp).toISOString() : 'unknown'} payload=${String(attack.payload || 'none').slice(0, 200)}\n\n` +
    `Return ONLY a JSON object with keys: executive_summary, technical_finding, likely_impact, remediation_steps (array), next_steps, risk_level. No markdown.`;
  try {
...
async function correlate(attacks) {
  ...
  attacks.forEach(a => {
    ...
    if (!byIp[ip]) byIp[ip] = [];
    byIp[ip].push({ type: a.attackType, severity: a.severity, status: a.status, ts: a.timestamp });
  });
  const topIps = Object.entries(byIp).sort((a, b) => b[1].length - a[1].length).slice(0, 10)
    .map(([ip, events]) => ({ ip, count: events.length, types: [...new Set(events.map(e => e.type))], severities: [...new Set(events.map(e => e.severity))], firstSeen: events.map(e => e.ts).filter(Boolean).sort()[0], lastSeen: events.map(e => e.ts).filter(Boolean).sort().reverse()[0] }));
  const multiTypeIps = topIps.filter(x => x.types.length > 1);
  const clusterSummary = topIps.map(x => `IP ${x.ip}: ${x.count} attacks, types=[${x.types.join(',')}], severity=[${x.severities.join(',')}]`).join('\n');
  const staticFallback = {
    campaigns: multiTypeIps.map(x => ({ name: `Campaign from ${x.ip}`, sourceIps: [x.ip], attackTypes: x.types, severity: x.severities.includes('critical') ? 'critical' : 'high', eventCount: x.count, firstSeen: x.firstSeen, lastSeen: x.lastSeen, assessment: `Multi-vector attacker: ${x.types.join(', ')}` })),
    sharedInfrastructure: [], attackChains: [],
    riskScore: Math.min(100, multiTypeIps.length * 20 + topIps.length * 5),
    summary: `Analysed ${attacks.length} attacks from ${Object.keys(byIp).length} unique IPs.`,
    generated: false,
  };
  ...
  const prompt =
    ...
    `Identify coordinated attack campaigns, shared attacker infrastructure, and attack chains.\n\n` +
    `Return ONLY a JSON object: { "campaigns": [{ "name": string, "sourceIps": string[], "attackTypes": string[], "severity": string, "eventCount": number, "firstSeen": string|null, "lastSeen": string|null, "assessment": string }], "sharedInfrastructure": [], "attackChains": [], "riskScore": number, "summary": string }\nNo markdown.`;
  try {
    const text = await generateWithFallback(prompt);
...
async function mutate(payload, attackType) {
  const staticMutations = [
    ...
    { variant: [...payload].map(c => `&#${c.charCodeAt(0)};`).join(''), technique: 'HTML Entity Encoding', evades: 'Plain-text WAF rules', risk: 'high', evasionProbability: 0.68, category: 'encoding' },
  ...
  const prompt =
    ...
    `Generate exactly 5 evasion variants. Return ONLY: { "mutations": [{ "variant": string, "technique": string, "evades": string, "risk": string, "evasionProbability": number, "category": string }] }\nExactly 5 items. No markdown.`;
  try {
    const text = await generateWithFallback(prompt);
    if (!text) return { original: payload, mutations: staticMutations, generated: false };
    const parsed = safeParseJSON(text);
