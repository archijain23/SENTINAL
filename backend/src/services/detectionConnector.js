...
const isCircuitOpen = () => {
  ...
  if (Date.now() - circuitOpenedAt > CIRCUIT_RESET_MS) {
    ...
    logger.info('[DETECTION] Circuit breaker reset');
    return false;
  }
  return true;
};
...
const openCircuit = () => {
  ...
  logger.warn('[DETECTION] Circuit breaker OPEN');
};

const analyze = async (logData) => {
  if (isCircuitOpen()) { logger.warn('[DETECTION] Circuit open — skipping'); return null; }
  try {
    const result = await withRetry(async () => {
      const response = await axios.post(
        `${DETECTION_ENGINE_URL}/analyze`,
        { logId: logData._id, projectId: logData.projectId, method: logData.method, url: logData.url, ip: logData.ip, queryParams: logData.queryParams, body: logData.body, headers: logData.headers },
