...
const callNexus = async (attack) => {
  try {
    const response = await axios.post(`${NEXUS_URL}/respond`, {
...
const reportAttack = async (data) => {
  ...
  if (['high', 'critical'].includes(data.severity)) {
    ...
    const alert = await Alert.create({
      ...
      meta:     { attackType: data.attackType, confidence: data.confidence, country: data.geoIntel?.country || null, country_code: data.geoIntel?.country_code || null, abuse_confidence_score: data.geoIntel?.abuse_confidence_score || 0, is_tor: data.geoIntel?.is_tor || false }
    });
...
const getRecentAttacks = async (limit = 20) => {
  return AttackEvent.find({}).sort({ createdAt: -1 }).limit(limit).select('-__v');
};
