...
const getStats = async () => {
  const [totalLogs, totalAttacks, totalAlerts, unreadAlerts, attacksByType, attacksBySeverity, recentAttacks] = await Promise.all([
    SystemLog.countDocuments(),
    AttackEvent.countDocuments(),
    Alert.countDocuments(),
    Alert.countDocuments({ isRead: false }),
    AttackEvent.aggregate([{ $group: { _id: '$attackType', count: { $sum: 1 } } }]),
    AttackEvent.aggregate([{ $group: { _id: '$severity',   count: { $sum: 1 } } }]),
    AttackEvent.find({}).sort({ createdAt: -1 }).limit(10).select('ip attackType severity status detectedBy confidence createdAt'),
  ]);
  return {
