...
const ingestAudit = async (req, res) => {
  try {
    const { intent_id, action, status, reason, policy_rule_id, enforcement_level, triggeredBy, ip, attackId, meta } = req.body;
    if (!action || !status) {
      return res.status(400).json({ success: false, message: 'action and status are required', code: 'VALIDATION_ERROR' });
    }
    const normStatus = (status || '').toUpperCase();
    const allowed    = ['ALLOWED', 'BLOCKED', 'APPROVED', 'REJECTED'];
    if (!allowed.includes(normStatus)) {
      return res.status(400).json({ success: false, message: `status must be one of ${allowed.join(', ')}`, code: 'VALIDATION_ERROR' });
    }
    ...
    emitter.emit(EVENTS.AUDIT_NEW, { id: entry._id, action: entry.action, status: entry.status, reason: entry.reason, policy_rule_id: entry.policy_rule_id, triggeredBy: entry.triggeredBy, ip: entry.ip, attackId: entry.attackId, timestamp: entry.createdAt });
...
const getAuditLog = async (req, res) => {
  try {
    ...
    const entries = await AuditLog.find({}).sort({ createdAt: -1 }).limit(limit).select('-__v');
