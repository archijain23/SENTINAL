...
const { chat, chatStream, generateReport, correlate, mutate } = require('../services/geminiService');
...
router.post('/chat', geminiLimiter, async (req, res) => {
  ...
  const safeHistory = Array.isArray(history)
    ? history.filter(h => h && typeof h.role === 'string' && typeof h.text === 'string').slice(-10)
    : [];
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: { answer: result.answer, grounded: result.grounded, errorCode: result.errorCode || null,
              suggestions: result.suggestions || [], sourcedEventIds: result.sourcedEventIds || [], context_attacks: attacks.length },
    });
  } catch (err) {
...
router.get('/chat/stream', geminiLimiter, async (req, res) => {
  ...
  if (!message || typeof message !== 'string' || !message.trim()) {
    res.status(400).json({ success: false, message: 'message query param is required', code: 'VALIDATION_ERROR' }); return;
  }
  let safeHistory = [];
  try { if (req.query.history) safeHistory = JSON.parse(req.query.history).slice(-10); } catch { /* ignore */ }
  ...
  try {
    ...
    logger.error(`[GeminiRoute] /chat/stream error: ${err.message}`);
...
router.post('/report/:attackId', geminiLimiter, async (req, res) => {
  ...
  const reportType = ['technical', 'executive', 'forensic'].includes(req.body?.reportType) ? req.body.reportType : 'technical';
  ...
  try {
    ...
    return res.status(200).json({ success: true, data: { report, attackId, reportType, generatedAt: new Date().toISOString() } });
  } catch (err) {
...
router.get('/report/:attackId/export', async (req, res) => {
  const { attackId } = req.params;
  const reportType = ['technical', 'executive', 'forensic'].includes(req.query?.reportType) ? req.query.reportType : 'technical';
  if (!attackId || attackId.length !== 24)
    return res.status(400).json({ success: false, message: 'Invalid attackId', code: 'VALIDATION_ERROR' });
  try {
    const attack = await AttackEvent.findById(attackId).lean();
    ...
    const md = [
      ...
      `**Attack ID:** ${attackId}`, `**Report Type:** ${reportType.toUpperCase()}`,
      `**Generated:** ${new Date().toISOString()}`, `**Risk Level:** ${report.risk_level?.toUpperCase() || 'UNKNOWN'}`,
      `**AI Generated:** ${report.generated ? 'Yes' : 'No (static fallback)'}`, ``, `---`, ``,
      `## Executive Summary`, ``, report.executive_summary || '_Not available_', ``,
      `## Technical Finding`, ``, report.technical_finding || '_Not available_', ``,
      `## Likely Impact`, ``, report.likely_impact || '_Not available_', ``,
      `## Remediation Steps`, ``,
      ...(Array.isArray(report.remediation_steps) ? report.remediation_steps.map((s, i) => `${i + 1}. ${s}`) : ['_Not available_']),
      ``, `## Next Steps`, ``, report.next_steps || '_Not available_', ``, `---`, ``,
    ...
    logger.error(`[GeminiRoute] /report/export error: ${err.message}`);
...
router.post('/correlate', geminiLimiter, async (req, res) => {
  try {
    ...
    if (attacks.length === 0)
      return res.status(200).json({ success: true, data: { campaigns: [], sharedInfrastructure: [], attackChains: [],
        riskScore: 0, summary: 'No attack data available to correlate.', attackCount: 0, generated: false } });

    const result = await correlate(attacks);
    try {
      await CorrelationSnapshot.create({ riskScore: result.riskScore || 0, summary: result.summary || '',
        attackCount: attacks.length, campaignCount: result.campaigns?.length || 0, generated: result.generated || false });
    } catch (snapErr) { logger.warn(`[GeminiRoute] snapshot save failed: ${snapErr.message}`); }

    return res.status(200).json({ success: true, data: { ...result, attackCount: attacks.length } });
  } catch (err) {
...
router.get('/correlate/history', async (req, res) => {
  try {
    const snapshots = await CorrelationSnapshot.find().sort({ createdAt: -1 }).limit(20).lean();
    return res.status(200).json({ success: true, data: snapshots.reverse() });
  } catch (err) {
