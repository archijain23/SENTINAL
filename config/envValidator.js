 ...
 * Called ONCE at the very beginning of server.js.
 */

'use strict';

const REQUIRED_VARS = [
  { name: 'MONGO_URI',       description: 'MongoDB connection string',                          example: 'mongodb+srv://user:pass@cluster.mongodb.net/sentinal' },
  { name: 'GATEWAY_PORT',    description: 'Port for the Express Gateway to listen on',           example: '3000' },
  { name: 'DETECTION_PORT',  description: 'Port for the Detection Engine (Python FastAPI)',      example: '8002' },
  { name: 'PCAP_PORT',       description: 'Port for the PCAP Processor (Python FastAPI)',        example: '8003' },
  { name: 'NEXUS_PORT',      description: 'Port for the SENTINAL Response Engine (Python FastAPI)', example: '8004' },
  { name: 'JWT_SECRET',      description: 'Secret key for JWT token signing',                   example: 'a_long_random_string_min_32_chars' }
];
...
function validateEnv({ exitOnFailure = true } = {}) {
  ...
  for (const v of REQUIRED_VARS) {
    ...
    if (!value || value.trim() === '') missing.push(v.name);
  }
  ...
  if (env.NODE_ENV === 'production') {
    for (const v of INSECURE_DEFAULTS) {
      ...
      if (v.insecureValues.includes(value)) {
        warnings.push(`${v.name} is set to a known insecure placeholder value. Change it before going live.`);
      }
    }
  }
